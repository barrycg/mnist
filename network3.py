# -*- coding: utf-8 -*-
"""
Created on Sun Jul 14 11:23:45 2019

it's a theano-based program for training and running simple neural networks.
...
GPU = True
if GPU:
    print("Trying to run under a GPU. If this is not desired, the modify"+ \
...
class Network(object):
    def __init__(self, layers, mini_batch_size):
        ...
        for j in range(1, len(self.layers)):
            ...
            layer.set_inpt(pre_layer.output, pre_layer.output_dropout, self.mini_batch_size)
            ...
            for minibatch_index in range(num_training_batches):
                ...
                if(iteration+1)%num_training_batches == 0:
                    validation_accuracy = np.mean(
                            [validation_mb_accuracy(j) for j in range(num_validation_batches)])
...
class ConvPoolLayer(object):
    ...
    def set_input(self, inpt, inpt_droppiut, mini_batch_size):
        self.inpt = inpt.reshape(self.image_shape)
        conv_out = conv.conv2d(
                input=self.inpt, filter=self.w, filter_shape=self.filter_shape,
